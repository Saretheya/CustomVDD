...
static constexpr DWORD IDD_SAMPLE_MONITOR_COUNT = 1; // CustomVDD: single stable virtual display

// CustomVDD: EDID-less fallback mode list (same set as pModeList).
...
// CustomVDD: 自定义 EDID（厂商码 CVD、名称 CustomVDD、1440p/1080p 时序）
// 使 Sunshine 等程序能稳定识别本虚拟显示器，而非通用的 "Generic Monitor"
static const struct IndirectSampleMonitor s_SampleMonitors[] =
{
    {
        { // pEdidBlock (128 bytes)

        },
        { // pModeList (24 modes: 1080p/1440p/3440x1440/4K x 60-240Hz)
{ 2560, 1440, 144 },
            { 2560, 1440,  60 },
            { 2560, 1440,  90 },
            { 2560, 1440, 120 },
        ...
        0  // ulPreferredModeIdx (2560x1440@144)
    }
};
static constexpr DWORD s_SampleMonitorCount = 1;
...
EVT_WDF_DRIVER_DEVICE_ADD CustomVDDDeviceAdd;
EVT_WDF_DEVICE_D0_ENTRY CustomVDDDeviceD0Entry;

EVT_IDD_CX_ADAPTER_INIT_FINISHED CustomVDDAdapterInitFinished;
EVT_IDD_CX_ADAPTER_COMMIT_MODES CustomVDDAdapterCommitModes;

EVT_IDD_CX_PARSE_MONITOR_DESCRIPTION CustomVDDParseMonitorDescription;
EVT_IDD_CX_MONITOR_GET_DEFAULT_DESCRIPTION_MODES CustomVDDMonitorGetDefaultModes;
EVT_IDD_CX_MONITOR_QUERY_TARGET_MODES CustomVDDMonitorQueryModes;

EVT_IDD_CX_MONITOR_ASSIGN_SWAPCHAIN CustomVDDMonitorAssignSwapChain;
EVT_IDD_CX_MONITOR_UNASSIGN_SWAPCHAIN CustomVDDMonitorUnassignSwapChain;
...
_Use_decl_annotations_
NTSTATUS CustomVDDDeviceAdd(WDFDRIVER Driver, PWDFDEVICE_INIT pDeviceInit)
{
    ...
    PnpPowerCallbacks.EvtDeviceD0Entry = CustomVDDDeviceD0Entry;
    ...
    // IddConfig.EvtIddCxDeviceIoControl = CustomVDDIoDeviceControl;

    IddConfig.EvtIddCxAdapterInitFinished = CustomVDDAdapterInitFinished;

    IddConfig.EvtIddCxParseMonitorDescription = CustomVDDParseMonitorDescription;
    IddConfig.EvtIddCxMonitorGetDefaultDescriptionModes = CustomVDDMonitorGetDefaultModes;
    IddConfig.EvtIddCxMonitorQueryTargetModes = CustomVDDMonitorQueryModes;
    IddConfig.EvtIddCxAdapterCommitModes = CustomVDDAdapterCommitModes;
    IddConfig.EvtIddCxMonitorAssignSwapChain = CustomVDDMonitorAssignSwapChain;
    IddConfig.EvtIddCxMonitorUnassignSwapChain = CustomVDDMonitorUnassignSwapChain;
...
_Use_decl_annotations_
NTSTATUS CustomVDDDeviceD0Entry(WDFDEVICE Device, WDF_POWER_DEVICE_STATE PreviousState)
{
...
void IndirectDeviceContext::InitAdapter()
{
    ...
    AdapterCaps.EndPointDiagnostics.pEndPointFriendlyName = L"CustomVDD Device";
    AdapterCaps.EndPointDiagnostics.pEndPointManufacturerName = L"Microsoft";
    AdapterCaps.EndPointDiagnostics.pEndPointModelName = L"CustomVDD Model";
...
void IndirectDeviceContext::FinishInit(UINT ConnectorIndex)
{
    ...
    if (ConnectorIndex >= s_SampleMonitorCount)
    {
    ...
    // ==============================
    // CustomVDD: 固定 ContainerId（替代示例代码的随机 GUID）
    //
    // 为什么必须固定：
    //   示例驱动每次启动都调用 CoCreateGuid() 生成随机 ContainerId，而 Windows
    //   正是用 ContainerId 判断"这是不是同一台显示器"。随机值导致每次驱动加载
    //   （开机 / 重装 / 升级）都被当作一台全新显示器，于是在
    //   DISPLAY\<硬件ID> 下不断新建实例（&0、&1、&2 …）。旧实例在父设备消失后
    //   残留为 Disconnected 幽灵设备，并使 \\.\DISPLAYnn 编号持续膨胀。
    //   实测本机曾累积 5 个幽灵实例，编号从 DISPLAY25 膨胀到 DISPLAY53。
    //   同一份示例代码的 ZakoVDD 也留下 1 个幽灵，属示例固有缺陷。
    //   （示例自身的注释亦建议 "it's best practice to choose a stable container ID"）
    //
    // 做法：
    //   使用由 EDID 身份派生的固定 GUID，每次加载都得到同一台显示器，
    //   Windows 复用既有实例，不再产生幽灵。
    //
    // 基准值来源：EDID 厂商码 CVD / 产品码 0x0001 / 序列号 0x00000001
    //   Data1 = 'C','V','D','0' = 0x43564430
    //   Data2 = 产品码 0x0001
    //   Data3 = 序列号低 16 位 0x0001
    ...
    MonitorInfo.MonitorContainerId.Data4[0] = 0x43;      // 'C'
    MonitorInfo.MonitorContainerId.Data4[1] = 0x56;      // 'V'
    MonitorInfo.MonitorContainerId.Data4[2] = 0x44;      // 'D'
    MonitorInfo.MonitorContainerId.Data4[3] = 0x00;
    MonitorInfo.MonitorContainerId.Data4[4] = 0x00;
    MonitorInfo.MonitorContainerId.Data4[5] = 0x00;
    MonitorInfo.MonitorContainerId.Data4[6] = 0x00;
    // 连接器索引：多连接器时保证唯一（本驱动 MonitorCount = 1，仅作稳健处理）
    MonitorInfo.MonitorContainerId.Data4[7] = (BYTE)(ConnectorIndex & 0xFF);
...
_Use_decl_annotations_
NTSTATUS CustomVDDAdapterInitFinished(IDDCX_ADAPTER AdapterObject, const IDARG_IN_ADAPTER_INIT_FINISHED* pInArgs)
{
...
_Use_decl_annotations_
NTSTATUS CustomVDDAdapterCommitModes(IDDCX_ADAPTER AdapterObject, const IDARG_IN_COMMITMODES* pInArgs)
{
...
_Use_decl_annotations_
NTSTATUS CustomVDDParseMonitorDescription(const IDARG_IN_PARSEMONITORDESCRIPTION* pInArgs, IDARG_OUT_PARSEMONITORDESCRIPTION* pOutArgs)
{
    // ==============================
    ...
    else
    {
        ...
        for(; SampleMonitorIdx < s_SampleMonitorCount; SampleMonitorIdx++)
        {
...
_Use_decl_annotations_
NTSTATUS CustomVDDMonitorGetDefaultModes(IDDCX_MONITOR MonitorObject, const IDARG_IN_GETDEFAULTDESCRIPTIONMODES* pInArgs, IDARG_OUT_GETDEFAULTDESCRIPTIONMODES* pOutArgs)
{
...
_Use_decl_annotations_
NTSTATUS CustomVDDMonitorQueryModes(IDDCX_MONITOR MonitorObject, const IDARG_IN_QUERYTARGETMODES* pInArgs, IDARG_OUT_QUERYTARGETMODES* pOutArgs)
{
    UNREFERENCED_PARAMETER(MonitorObject);
    ...
    TargetModes.push_back(CreateIddCxTargetMode(2560, 1440, 165));
    ...
    TargetModes.push_back(CreateIddCxTargetMode(1920, 1080, 165));
    TargetModes.push_back(CreateIddCxTargetMode(1920, 1080, 240));
    TargetModes.push_back(CreateIddCxTargetMode(3440, 1440, 60));
    TargetModes.push_back(CreateIddCxTargetMode(3440, 1440, 90));
    TargetModes.push_back(CreateIddCxTargetMode(3440, 1440, 120));
    TargetModes.push_back(CreateIddCxTargetMode(3440, 1440, 144));
    TargetModes.push_back(CreateIddCxTargetMode(3440, 1440, 165));
    TargetModes.push_back(CreateIddCxTargetMode(3440, 1440, 240));
    ...
    TargetModes.push_back(CreateIddCxTargetMode(3840, 2160, 165));
    TargetModes.push_back(CreateIddCxTargetMode(3840, 2160, 240));
    TargetModes.push_back(CreateIddCxTargetMode(2560, 1440, 144));
    TargetModes.push_back(CreateIddCxTargetMode(2560, 1440, 90));
    TargetModes.push_back(CreateIddCxTargetMode(2560, 1440, 60));
    TargetModes.push_back(CreateIddCxTargetMode(1920, 1080, 144));
...
_Use_decl_annotations_
NTSTATUS CustomVDDMonitorAssignSwapChain(IDDCX_MONITOR MonitorObject, const IDARG_IN_SETSWAPCHAIN* pInArgs)
{
...
_Use_decl_annotations_
NTSTATUS CustomVDDMonitorUnassignSwapChain(IDDCX_MONITOR MonitorObject)
{
    auto* pMonitorContextWrapper = WdfObjectGet_IndirectMonitorContextWrapper(MonitorObject);
